...
const BankCard = ({ initialData, onSave }) => {
  const [enabled, setEnabled] = useState(initialData?.enabled ?? true);
  const [bankName, setBankName] = useState(
    initialData?.bankName || "ธนาคารไทยพาณิชย์ (SCB)"
  );
  ...
  const handleSave = (e) => {
    e?.preventDefault();
    if (onSave) {
      onSave({ enabled, bankName, accountNumber, accountName });
    }
    setIsSaved(true);
  ...
  return (
    <div className="rounded-2xl border border-[#2b2542] bg-[#16122a]/80 backdrop-blur-md p-6 sm:p-7 shadow-xl transition-all flex flex-col justify-between hover:border-purple-500/30">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        {/* Left Banner */}
        <div className="flex items-center gap-4 rounded-2xl bg-linear-to-r from-[#2c3e50] to-[#34495e] border border-slate-400/20 px-5 py-4 flex-1 shadow-md">
          {/* Bank Icon Badge */}
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white border border-white/20 shadow-sm">
            <Landmark size={24} />
          </div>
          <div>
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              >
                {bankList.map((b) => (
                  <option key={b} value={b} className="bg-[#16122a] text-white">
                    {b}
                  </option>
                ))}
              </select>
